...
def get_players_trending(type_, lookback_hours, limit):
    return fetch_data("/players/nfl/trending/{type}?lookback_hours={lookback_hours}&limit={limit}",
                        type=type_, lookback_hours=lookback_hours, limit=limit)
...
def get_league_transactions(league_id, round_):
    return fetch_data("/league/{league_id}/transactions/{round}", league_id=league_id, round=round_)
