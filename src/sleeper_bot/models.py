...
class Player(TimeStampedModel):
    ...
    player_id = models.CharField(
        primary_key=True, unique=True, db_index=True, max_length=250
    )
    ...
    team = models.CharField(
        choices=TeamChoices.choices, max_length=250, null=True, blank=True
    )
    position = models.CharField(max_length=250, null=True, blank=True)
    fantasy_positions = MultiSelectField(
        choices=FantasyPosition.choices, max_length=250, null=True, blank=True
    )
...
class PlayerMetadata(models.Model):
    player = models.OneToOneField(
        Player, on_delete=models.CASCADE, related_name="metadata"
    )
